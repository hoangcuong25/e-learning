import {
  Injectable,
  NotFoundException,
  ForbiddenException,
} from "@nestjs/common";
import { PrismaService } from "../../../core/prisma/prisma.service";
import { CreatePostDto } from "./dto/create-post.dto";
import { UpdatePostDto } from "./dto/update-post.dto";
import { PostQueryDto } from "./dto/post-query.dto";
import { PostVisibility } from "@prisma/client";
import {
  buildPaginationParams,
  buildOrderBy,
  buildPaginationResponse,
  buildSearchFilter,
} from "../../../core/helpers/pagination.util";

@Injectable()
export class PostService {
  constructor(private readonly prisma: PrismaService) {}

  async create(createPostDto: CreatePostDto, userId: number) {
    const { media, ...postData } = createPostDto;

    return this.prisma.post.create({
      data: {
        ...postData,
        authorId: userId,
        type: "POST",
        media: {
          create: media,
        },
      },
      include: {
        media: true,
        author: {
          select: {
            id: true,
            fullname: true,
            avatar: true,
          },
        },
      },
    });
  }

  async findAll(currentUserId: number | null, query: PostQueryDto) {
    const { authorId, visibility } = query;
    const { skip, take, page, limit } = buildPaginationParams(query);
    const orderBy = buildOrderBy(query); // createdAt: desc

    // Single Query for ALL posts (POST and SHARE)
    const where: any = {};
    const searchFilter = buildSearchFilter(query, ["content"]);
    if (searchFilter) Object.assign(where, searchFilter);

    if (authorId) where.authorId = authorId;

    // Visibility filter logic
    const visibilityFilters: any[] = [{ visibility: PostVisibility.PUBLIC }];

    if (currentUserId) {
      // Author can always see their own posts (including PRIVATE and FOLLOWERS)
      visibilityFilters.push({ authorId: currentUserId });

      // Followers can see FOLLOWERS posts
      visibilityFilters.push({
        visibility: PostVisibility.FOLLOWERS,
        author: {
          followers: {
            some: {
              followerId: currentUserId,
            },
          },
        },
      });
    }

    if (visibility) {
      // If user explicitly filters by visibility, we must still respect access rules
      where.AND = [
        ...(where.AND || []),
        { visibility },
        { OR: visibilityFilters },
      ];
    } else {
      // Default: show anything the user is allowed to see
      where.OR = visibilityFilters;
    }

    const [posts, total] = await Promise.all([
      this.prisma.post.findMany({
        where,
        skip,
        take,
        orderBy,
        include: {
          media: true,
          author: {
            select: {
              id: true,
              fullname: true,
              avatar: true,
              role: true,
            },
          },
          // Include Original Post if it exists (for shares)
          originalPost: {
            include: {
              media: true,
              author: {
                select: {
                  id: true,
                  fullname: true,
                  avatar: true,
                },
              },
              _count: {
                select: {
                  likes: true,
                  comments: true,
                  sharedPosts: true,
                },
              },
              ...(currentUserId && {
                likes: {
                  where: { userId: currentUserId },
                  select: { id: true },
                },
              }),
            },
          },
          _count: {
            select: {
              likes: true,
              comments: true,
              sharedPosts: true,
            },
          },
          ...(currentUserId && {
            likes: {
              where: { userId: currentUserId },
              select: { id: true },
            },
          }),
        },
      }),
      this.prisma.post.count({ where }),
    ]);

    // Format Result
    const postsResult = posts.map((post) => ({
      ...post,
      isLiked: currentUserId ? post.likes?.length > 0 || false : false,
      likes: undefined,

      // Format originalPost if it exists
      originalPost: post.originalPost
        ? {
            ...post.originalPost,
            isLiked: currentUserId
              ? post.originalPost.likes?.length > 0 || false
              : false,
            likes: undefined,
          }
        : null,
    }));

    return {
      data: {
        posts: postsResult,
      },
      pagination: {
        currentPage: page,
        pageSize: limit,
        total,
        totalPages: Math.ceil(total / limit),
      },
    };
  }

  async findOne(id: number, userId: number | null) {
    const post = await this.prisma.post.findUnique({
      where: { id },
      include: {
        media: true,
        author: {
          select: {
            id: true,
            fullname: true,
            avatar: true,
          },
        },
        _count: {
          select: {
            likes: true,
            comments: true,
            sharedPosts: true,
          },
        },
      },
    });

    if (!post) {
      throw new NotFoundException(`Bài viết có id: ${id} không tồn tại`);
    }

    if (
      post.visibility === "PRIVATE" &&
      (!userId || post.authorId !== userId)
    ) {
      throw new ForbiddenException("Bạn chỉ có thể truy cập bài viết của mình");
    }

    return post;
  }

  async update(id: number, updatePostDto: UpdatePostDto, userId: number) {
    const post = await this.prisma.post.findUnique({ where: { id } });
    if (!post) throw new NotFoundException("Không tìm thấy bài viết");

    if (post.authorId !== userId) {
      throw new ForbiddenException("Bạn chỉ có thể cập nhật bài viết của mình");
    }

    const { media, ...data } = updatePostDto;

    if (media) {
      await this.prisma.postMedia.deleteMany({ where: { postId: id } });
    }

    return this.prisma.post.update({
      where: { id },
      data: {
        ...data,
        media: media
          ? {
              create: media,
            }
          : undefined,
      },
      include: {
        media: true,
      },
    });
  }

  async remove(id: number, userId: number, userRole: string) {
    const post = await this.prisma.post.findUnique({ where: { id } });

    if (!post) {
      throw new NotFoundException("Không tìm thấy bài viết");
    }

    if (post.authorId !== userId && userRole !== "ADMIN") {
      throw new ForbiddenException("Bạn chỉ có thể xóa bài viết của mình");
    }

    return this.prisma.post.delete({
      where: { id },
    });
  }

  async toggleLike(postId: number, userId: number) {
    const existingLike = await this.prisma.postLike.findUnique({
      where: {
        userId_postId: {
          userId,
          postId,
        },
      },
    });

    if (existingLike) {
      await this.prisma.postLike.delete({
        where: { id: existingLike.id },
      });
      return { message: "Đã bỏ thích" };
    } else {
      await this.prisma.postLike.create({
        data: {
          userId,
          postId,
        },
      });
      return { message: "Đã thích" };
    }
  }

  async share(postId: number, userId: number, content?: string) {
    const original = await this.prisma.post.findUnique({
      where: { id: postId },
    });
    if (!original) throw new NotFoundException("Không tìm thấy bài viết");

    const targetId = original.originalPostId || postId;

    return this.prisma.post.create({
      data: {
        type: "SHARE",
        content,
        authorId: userId,
        originalPostId: targetId,
        visibility: "PUBLIC",
      },
      include: {
        author: {
          select: { id: true, fullname: true, avatar: true },
        },
        originalPost: {
          include: {
            media: true,
            author: { select: { id: true, fullname: true, avatar: true } },
          },
        },
      },
    });
  }

  async updateVisibility(
    postId: number,
    userId: number,
    visibility: PostVisibility
  ) {
    const post = await this.prisma.post.findUnique({
      where: { id: postId },
    });

    if (!post) {
      throw new NotFoundException("Không tìm thấy bài viết");
    }

    if (post.authorId !== userId) {
      throw new ForbiddenException(
        "Bạn chỉ có thể thay đổi quyền riêng tư của bài viết của mình"
      );
    }

    return this.prisma.post.update({
      where: { id: postId },
      data: { visibility },
      include: {
        media: true,
        author: {
          select: {
            id: true,
            fullname: true,
            avatar: true,
          },
        },
      },
    });
  }
}
