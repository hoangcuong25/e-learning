    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
          Hiện chưa có đơn ứng tuyển nào mới
        </p>
      </div>
    );
  }
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 text-slate-500 mt-0.5">
                        ...
                        <span className="text-[11px] font-medium tracking-tight truncate max-w-[150px] sm:max-w-none">
                          {app.user?.email}
                        </span>
                      </div>
                    </div>
              ...
              <CardContent className="p-8 pt-2 space-y-6 relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-800/50">
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                      <Briefcase size={12} className="text-indigo-500" /> Kinh
                      nghiệm
                    </div>
                    <p className="text-sm font-bold text-slate-300 pl-4">
                      {app.experience}
                    </p>
                  </div>
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 pl-4 mt-1">
                      {app.applicationSpecializations?.map((item, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-0.5 bg-slate-950 border border-slate-800 rounded-lg text-[10px] font-bold text-indigo-400"
                        >
                ...
                <div className="p-5 bg-slate-950/40 border border-slate-800/50 rounded-2xl relative group-hover:bg-slate-950 transition-colors">
                  <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-600 uppercase tracking-widest mb-3">
                    <Info size={12} className="text-indigo-400" /> Giới thiệu
                    ứng viên
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-slate-800/50">
                  <button
                    disabled={
                      app.user?.id === undefined || app.status !== "PENDING"
                    }
                    onClick={() =>
                      app.user?.id && handleApproveApp(app.user.id, app.id)
                    }
                  ...
                  >
                    <Check size={14} className="stroke-[3]" /> Chấp thuận đăng
                    ký
                  </button>
                  <button
                    disabled={
                    ...
                    onClick={() =>
                      app.user?.id && handleRejectApp(app.user.id, app.id)
                    }
