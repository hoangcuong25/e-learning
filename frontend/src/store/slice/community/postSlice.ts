...
import {
  ...
  toggleLikePostApi,
  sharePostApi,
  updatePostVisibilityApi,
} from "@/store/api/community/post.api";

interface PostState {
  posts: any[]; // (PostType | PostShareType)[];
  currentPost: any | null; // PostType | null;
  pagination: {
    total: number;
    totalPages: number;
    currentPage: number;
...
export const updatePostVisibility = createAsyncThunk(
  ...
  async (data: { id: number; visibility: string }, { rejectWithValue }) => {
    try {
      const response = await updatePostVisibilityApi(data.id, data.visibility);
      return response;
    } catch (error: any) {
      return rejectWithValue(
        error.response?.data || "Lỗi cập nhật quyền riêng tư"
      );
    }
  }
);
...
const postSlice = createSlice({
    ...
    incrementCommentCount: (state, action) => {
      ...
      if (state.currentPost && state.currentPost.id === postId) {
        if (state.currentPost._count) {
          state.currentPost._count.comments =
            (state.currentPost._count.comments || 0) + 1;
        }
      }
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      // 🔒 Update Visibility
      .addCase(updatePostVisibility.pending, (state) => {
        state.loading = true;
      })
      .addCase(updatePostVisibility.fulfilled, (state, action) => {
        state.loading = false;
        state.successMessage = "Cập nhật quyền riêng tư thành công";
      ...
      .addCase(updatePostVisibility.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Lỗi khi cập nhật quyền riêng tư";
      });
  },
});

export const { clearPostState, setPosts, incrementCommentCount } =
  postSlice.actions;
