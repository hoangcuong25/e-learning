      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-white p-6 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-400 font-medium max-w-sm mx-auto">
            Giảng viên đang nỗ lực hoàn thiện nội dung. Vui lòng quay lại sau!
          </p>
        </div>
        <Button
              ...
              <div className="space-y-4 flex-1">
                <div className="flex items-center gap-3">
                  <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase tracking-widest">
                    Chương {currentLesson?.chapter?.id}
                  </span>
                  <span className="text-slate-300">•</span>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Clock size={12} /> {currentIndex + 1} / {lessons.length}{" "}
                    Bài học
                  </span>
                </div>
                ...
                <div className="bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 space-y-8">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-[10px] font-black bg-indigo-600/30 px-3 py-1 rounded-full uppercase tracking-widest">
                        Challenges
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-slate-400 text-sm font-medium leading-relaxed">
                        Hoàn thành bài kiểm tra để củng cố kiến thứcvừa học.
                      </p>
                    </div>

                    <div className="space-y-3">
                      {currentLesson?.quizzes?.length ? (
                        currentLesson?.quizzes?.map(
                          (quiz: any, idx: number) => (
                            <button
                              key={quiz.id}
                              onClick={() =>
                                router.push(
                                  `/learn/${courseId}/quiz/${quiz.id}`,
                                )
                              }
                            ...
                            >
                              <div className="flex items-center gap-4">
                                ...
                                <span className="text-sm font-bold tracking-tight group-hover/quiz:text-indigo-400 transition-colors">
                                  {quiz.title}
                                </span>
                              </div>
                              <ChevronRight
                                size={16}
                                className="text-slate-600 group-hover/quiz:translate-x-1 transition-transform"
                              />
                            </button>
                          ),
                        )
