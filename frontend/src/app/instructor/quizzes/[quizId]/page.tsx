    ...
    <div className="space-y-10 pb-10 overflow-x-hidden">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
             <div className="flex items-center gap-3">
                ...
                <div className="flex items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]"><Layers size={12} className="mr-2" /> Quiz Management</div>
             </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 group hover:shadow-xl transition-all duration-500">
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Khóa học hiện tại</span>
            </div>
            <h4 className="text-lg font-black text-slate-900 leading-tight truncate">{course?.title || "Draft Course"}</h4>
         </Card>
         ...
         <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 group hover:shadow-xl transition-all duration-500">
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Chương / Module</span>
            </div>
            <h4 className="text-lg font-black text-slate-900 leading-tight truncate font-bold">{chapter?.title || "Module #0"}</h4>
         </Card>
         ...
         <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 group hover:shadow-xl transition-all duration-500">
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Bài học áp dụng</span>
            </div>
            <h4 className="text-lg font-black text-slate-900 leading-tight truncate font-bold">{lesson?.title || "Intro Session"}</h4>
         </Card>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
           <div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">Ngân hàng câu hỏi</h2>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Sắp xếp và quản lý nội dung kiểm tra</p>
           </div>
        ...
        {currentQuiz && currentQuiz?.questions && currentQuiz?.questions?.length > 0 ? (
              ...
              >
                <CardHeader className="p-10 pb-6 border-b border-slate-50 flex flex-row items-start justify-between">
                  <div className="flex items-start gap-6">
                    ...
                    <div className="space-y-1">
                      <div className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Question Content</div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button variant="ghost" size="icon" className="w-12 h-12 rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-600 transition-all">
                          <Trash2 className="w-5 h-5" />
                        </Button>
                      </AlertDialogTrigger>
                      <AlertDialogContent className="rounded-[2.5rem] border-none p-10 bg-white shadow-2xl">
                        <AlertDialogHeader>
                          <AlertDialogTitle className="text-2xl font-black text-slate-900 tracking-tight leading-none">Gỡ câu hỏi?</AlertDialogTitle>
                          <AlertDialogDescription className="text-slate-500 font-medium py-4 text-base leading-relaxed">
                            Bạn có chắc chắn muốn gỡ bỏ câu hỏi này khỏi bài kiểm tra? Hành động này có thể ảnh hưởng đến kết quả của học viên đã thực hiện quiz.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter className="gap-4">
                          <AlertDialogCancel className="h-14 flex-1 rounded-2xl font-bold bg-slate-50 border-none hover:bg-slate-100 text-slate-500">Bỏ qua</AlertDialogCancel>
                          <AlertDialogAction
                    ...
                    {q.options && q.options.length > 0 ? (
                      q.options.map((opt, optIndex) => (
                        ...
                        >
                          ...
                          {opt.isCorrect && (
                            <Badge className="bg-emerald-500 text-white border-none rounded-lg px-2 py-0.5 text-[8px] font-black uppercase tracking-widest">Correct</Badge>
                          )}
                        </div>
                      ))
                    ) : (
                      <div className="col-span-full py-8 text-center bg-white/50 border-2 border-dashed border-slate-100 rounded-[2rem]">
                         ...
                         <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">No options defined</p>
                      </div>
                    )}
                  </div>
                </CardContent>
          ...
          <div className="py-24 flex flex-col items-center justify-center bg-white rounded-[3rem] shadow-sm border-2 border-dashed border-slate-100">
             ...
             <h3 className="text-xl font-black text-slate-900">Quiz chưa có nội dung</h3>
             <p className="text-slate-400 mt-1 font-medium">Bấm vào nút "Tạo câu hỏi" ở góc phải để bắt đầu thiết lập.</p>
          </div>
        )}
      </div>
    </div>
