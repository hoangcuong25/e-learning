      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Tối ưu hóa doanh thu bằng cách tạo các chiến dịch khuyến mãi hấp dẫn cho học viên.
          </p>
        </div>
        ...
        {instructorCoupons.length === 0 ? (
          <Card className="border-2 border-dashed border-slate-200 bg-slate-50/50 rounded-[2.5rem] shadow-none">
            <CardContent className="py-24 flex flex-col items-center text-center">
              ...
              <h3 className="text-2xl font-black text-slate-800 mb-3">Chưa có mã giảm giá nào</h3>
              <p className="text-slate-500 max-w-sm mb-10 font-medium italic">
                Sử dụng coupon để khuyến khích học viên đăng ký các khóa học của bạn ngay bây giờ!
              </p>
            </CardContent>
          </Card>
        ) : (
            ...
            {instructorCoupons.map((coupon) => (
              ...
              >
                ...
                <div className="w-32 sm:w-40 flex-shrink-0 bg-gradient-to-br from-emerald-500 to-teal-600 flex flex-col items-center justify-center relative overflow-hidden text-white">
                   ...
                   <div className="relative font-black text-4xl sm:text-5xl tracking-tighter shadow-sm">{coupon.percentage}%</div>
                   <div className="relative text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] mt-1 opacity-90">OFF</div>
                ...
                <div className="flex-1 p-6 sm:p-8 flex flex-col min-w-0">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                         <div className="text-lg sm:text-2xl font-black text-slate-900 tracking-tight">{coupon.code}</div>
                         <Badge className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border-none ${coupon.isActive ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-400"}`}>
                            {coupon.isActive ? "Active" : "Inactive"}
                         </Badge>
                      </div>
                      <p className="text-xs sm:text-sm text-slate-500 font-bold line-clamp-1">{coupon.title || "Tết Giáp Thìn 2024"}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                       ...
                       <AlertDialog>
                          ...
                          <AlertDialogContent className="rounded-[2.5rem] p-8 border-none shadow-2xl">
                            <AlertDialogHeader>
                              <AlertDialogTitle className="text-xl font-black text-slate-900">Xác nhận xóa coupon</AlertDialogTitle>
                              <AlertDialogDescription className="text-slate-500 font-medium leading-relaxed mt-2">
                                Bạn sắp xóa vĩnh viễn mã giảm giá <strong>{coupon.code}</strong>. Hành động này không thể hoàn tác.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter className="mt-6 gap-3">
                              <AlertDialogCancel className="h-12 px-6 rounded-2xl border-slate-100 text-slate-500 font-bold hover:bg-slate-50">Hủy bỏ</AlertDialogCancel>
                              <AlertDialogAction onClick={confirmDelete} className="h-12 px-8 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-tight">Đồng ý xóa</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                       </AlertDialog>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-auto">
                     <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100/50">
                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 leading-none">Hiệu năng</div>
                        <div className="flex items-center justify-between">
                           <span className="text-xs font-bold text-slate-600">Đã dùng:</span>
                           <span className="text-sm font-black text-indigo-600">{coupon.usedCount}</span>
                        </div>
                        <div className="flex items-center justify-between mt-1">
                           <span className="text-xs font-bold text-slate-600">Giới hạn:</span>
                           <span className="text-sm font-black text-slate-700">{coupon.maxUsage || "∞"}</span>
                        </div>
                     </div>

                     <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100/50">
                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 leading-none">Hết hạn</div>
                        {coupon.expiresAt ? (
                           <div className="text-[11px] font-black text-slate-700 leading-tight">
                              {new Date(coupon.expiresAt).toLocaleDateString("vi-VN", {day: '2-digit', month: '2-digit', year: 'numeric'})}
                              <br/>
                              <span className={`${new Date(coupon.expiresAt) < new Date() ? "text-red-500" : "text-emerald-500"}`}>
                                 {new Date(coupon.expiresAt) < new Date() ? "Đã quá hạn" : "Còn thời hạn"}
                              </span>
                           </div>
                        ) : (
                           <span className="text-xs font-black text-slate-700">Vô thời hạn</span>
                        )}
                     </div>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
                     <div className="flex items-center gap-2 overflow-hidden">
                        <div className="p-1 px-2.5 bg-slate-900 text-[10px] text-white font-black rounded-lg uppercase tracking-tight">Target</div>
                        <div className="text-[11px] font-bold text-slate-500 truncate italic">
                           {coupon.target === "COURSE" && coupon.course ? coupon.course.title : coupon.target === "SPECIALIZATION" && coupon.specialization ? coupon.specialization.name : "Tất cả học phần"}
                        </div>
                     </div>
                  </div>
                </div>
